...
export const ProjectHeader = ({ project, onArchive }: { project: Project; onArchive?: () => void }): ReactElement => <header className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-200 pb-6">
  ...
  <div className="flex gap-2"><Link className="button max-w-fit" to={`/projects/${project._id}/issues`}>Issues</Link>{onArchive && project.status !== 'ARCHIVED' && <button className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700" onClick={onArchive}>Archive</button>}<Link className="button max-w-fit" to={`/projects/${project._id}/settings`}>Settings</Link></div>
</header>;