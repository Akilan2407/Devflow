...
export const projectService = {
  async create(userId: string, input: CreateProjectInput): Promise<ProjectDocument> {
    ...
    return ProjectModel.create({ ...input, ownerId, members: memberIds, key: input.key.toUpperCase() });
  },

  async list(organizationId: string, options: { page: number; limit: number; search?: string; status?: string; sort: string }) {
